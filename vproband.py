...
if len(st.split(pro[8+Nions+1]))>1:     # spin-orbit coupling
    soc=4   # 4 blocks
else:
    soc=1   # 1 block
fmt=len(st.split(pro[7]))
if fmt!=5 and fmt!=11:
...
[Ne,Nk,Nbands]=map(int,st.split(eigenval[5]))
eigfile.close()
...
atom=raw_input("Input the atom: (Integer) for single atom, (S) for species:")
E_F=-0.764756
print "E_F", E_F

while 1:
    if fmt==5:
        spd=raw_input('Input the orbital: tot, s, p, d\n')
        if spd in ['s', 'p', 'd', 'tot']:
            break
        else:
            ...
            continue
    if fmt==11:
        spd=raw_input('Input the orbital: tot, s, p, d, py, pz, px, dxy, dyz, dz2, dxz, dx2\n')
...
if Nspin == 1:
    ...
    if atom[0] in ['s', 'S']:
        ...
        out=file('bands_pro_%s_%d.dat'%(spd,int(atom)),'w+')
    ...
    for i in range(Nbands):
        for j in range(len(kpoint)):
            ...
            if atom[0] in ['s', 'S']:
                ...
                out.write('    %f'%(pop[j][i][int(atom)-1]*pro_scale/totpop))
            out.write('\n')
        out.write('\n')
    out.close()
